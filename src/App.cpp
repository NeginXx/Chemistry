...
// FIXME
void DoEverything(const CoordinateSystem& coord_sys,
                    MoleculeBox& box,
  ...
  box.ProcessOneIteration(dt);
  box.Draw(coord_sys, render);
  manager.DrawButtons(coord_sys, render);
  // if (++cnt % 100 == 0) {
  //   plot.AddPoint(box.CalculateKineticEnergy(), dt);
  //   printf("%d %f\n", cnt, box.CalculateKineticEnergy());
  // }
  // plot.Draw(render, "t", "N");
  ...
  void RunApp(const Window& window, Render& render) {
    ...
    for (size_t i = 0; i < 50; ++i) {
      ...
      box.AddShape<PhysicalRectangle>(center, width, height, velocity * factor, mass, kRed);
    }
    // ------------------------------------------------------------------------------------
    ...
    while (is_running) {
      DrawEverything(coord_sys, box, button_manager, plot, render);
